...
describe('CustomGameSettings', () => {
  it('starts collapsed and summarizes the copies', async () => {
    const wrapper = mount(CustomGameSettings, {...globalConfig, attachTo: document.body, props: {cardCopies: {[CardName.COMET]: 3, [CardName.ALGAE]: 1}}});
  ...
  it('forwards copy changes', async () => {
    const wrapper = mount(CustomGameSettings, {...globalConfig, props: {cardCopies: {}}});
    await wrapper.find('[data-test=toggle]').trigger('click');
